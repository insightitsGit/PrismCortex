...
| Throughput (cached, c=20) | ~37–80 req/s | Network + ACI variance |
| Recall load (c=50, 2000 req) | Run split `bench_load()` | Cached reads only — isolates read SLO |
| Digest load (c=16, 400 req) | Run split `bench_load()` | Salience-skipped chatter; matches digest semaphore |
| Mixed smoke (c=20, 500 req) | Run split `bench_load()` | 20% writes — modest combined pressure |
...
queued behind digests on the default Starlette thread pool). Fixed by split load + read/write pools.
